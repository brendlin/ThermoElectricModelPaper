...
\subfloat[] {\label{fig:rampoptimization_b} \includegraphics[width=0.45\linewidth]{figures/ModulePower_R3_newramp.eps}}
\caption{(a) Sensor leakage current and (b) total power of the endcap R3-type module using different cooling profiles. The lines that are discontinued before year 14 correspond to scenarios that have reached thermal runaway. The dashed line represents the ramp scenario specified in Fig.~\ref{fig:coolant_ramp}, which has been selected to minimize the module power while keeping the sensor leakage current stable throughout the lifetime of the ITk.}
